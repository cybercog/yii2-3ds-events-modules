...
namespace yii3ds\events\controllers\backend;

use vova07\admin\components\Controller;
use yii3ds\events\models\backend\Event;
use yii3ds\events\models\backend\EventSearch;
...
class DefaultController extends Controller
{
    ...
    public function behaviors()
    {
        ...
        $behaviors['access']['rules'][] = [
            ...
            'roles' => ['BCreateEvents', 'BUpdateEvents']
        ];
        $behaviors['verbs'] = [
    ...
    public function actionIndex()
    {
        ...
        $statusArray = Event::getStatusArray();
    ...
    {
        $model = new Event(['scenario' => 'admin-create']);
        // $statusArray = Event::getStatusArray();
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger', Module::t('events', 'BACKEND_FLASH_FAIL_ADMIN_CREATE'));
                    return $this->refresh();
                }
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                if ($model->save(false)) {
                    ...
                    Yii::$app->session->setFlash('danger', Module::t('events', 'BACKEND_FLASH_FAIL_ADMIN_UPDATE'));
                    return $this->refresh();
                }
            } elseif (Yii::$app->request->isAjax) {
                Yii::$app->response->format = Response::FORMAT_JSON;
     ...
     *
     * @return \yii3ds\events\models\backend\Event Model
     *
    ...
    {
        if (is_array($id)) {
            /** @var \yii3ds\events\models\backend\Event $model */
            $model = Event::findAll($id);
        } else {
            /** @var \yii3ds\events\models\backend\Event $model */
